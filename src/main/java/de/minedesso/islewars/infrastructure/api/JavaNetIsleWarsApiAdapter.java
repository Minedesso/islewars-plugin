...
public final class JavaNetIsleWarsApiAdapter implements IsleWarsApiPort {
    ...
    public JavaNetIsleWarsApiAdapter(HttpClient httpClient, IsleWarsApiConfiguration configuration) {
        this(httpClient, configuration, new ObjectMapper()
                .configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false));
    }

    JavaNetIsleWarsApiAdapter(HttpClient httpClient, IsleWarsApiConfiguration configuration,
    ...
    public CompletableFuture<ServerConfiguration> fetchServerConfiguration() {
        ...
        return this.httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString(StandardCharsets.UTF_8))
                .thenApply(response -> {
                    ...
                    try {
                        return this.objectMapper.readValue(response.body(), ServerConfigurationResponse.class).toDomain();
    ...
    private HttpRequest.Builder requestBuilder(URI uri) {
        if (this.configuration.clientName().isBlank() || this.configuration.apiKey().isBlank()) {
            throw new IsleWarsApiException("Für die IsleWars-API sind NAME und API-KEY erforderlich.");
        }
        return HttpRequest.newBuilder(uri)
                .timeout(this.configuration.requestTimeout())
                .header("Accept", "application/json")
                .header(NAME_HEADER, this.configuration.clientName())
    ...
    private record ServerConfigurationResponse(String mode, int minimumPlayers, int maximumPlayers,
                                               int countdownSeconds, LobbySpawnPayload lobbySpawn) {
        private ServerConfiguration toDomain() {
            return new ServerConfiguration(IsleWarsMode.fromApiValue(this.mode), this.minimumPlayers,
                    this.maximumPlayers, this.countdownSeconds,
                    this.lobbySpawn == null ? null : this.lobbySpawn.toDomain());
        }
    }

    private record LobbySpawnPayload(String worldName, double x, double y, double z, float yaw, float pitch) {
